...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        ...
        let path_buf = fs::read_link(path).ok()?;
        let fd_name = OpenFile::path_to_name(path_buf.to_str()?);
        ...
        Some(OpenFile::new(
            fd_name,
            OpenFile::parse_cursor(&fdinfo_content)?,
            OpenFile::parse_access_mode(&fdinfo_content)?,
        ))
    }
